"""Добавляет адаптацию по видимым значениям, сохраняя универсальную модель."""
...
def examples(frame, seed):
    ...
    x["field_code"] = (
        original.anon_polygon_id.str.split("-").str[-1].astype(int).to_numpy()
    )
    labels = np.where(
...
def main():
    ...
    if (
        len(report.get("folds", {})) != 4
        or report["aggregate"]["mixed"]["rmse"] >= report["aggregate"]["v15"]["rmse"]
    ):
    ...
    parts = []
    weights = []
    for label, data, seeds, weight in [
        ("reference", reference, [142, 243, 344, 445, 546, 647], 1.0),
        ("visible_test", clean, [771, 882, 993, 1104, 1215, 1326], 3.0),
    ]:
        for seed in seeds:
    ...
    if artifact.get("kind") != "source_experts_v1":
        raise ValueError("Source-expert base model required.")
    artifact["adaptation"] = {
        "inputSha256": hashlib.sha256(Path(args.input).read_bytes()).hexdigest(),
        "fieldCodes": {
            str(v): int(str(v).split("-")[-1]) for v in frame.anon_polygon_id.unique()
        },
        "bundle": bundle,
        "weight": 0.5,
        "examples": len(y),
    ...
    print(
        ...
        json.dumps(
            {
                "inputSha256": artifact["adaptation"]["inputSha256"],
                "examples": len(y),
