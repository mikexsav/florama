"""Восстановление с экспертами источников только по видимым наблюдениям."""
...
def extended_features(frame, base):
    ...
    for _, group in frame.assign(_year=dates.dt.year).groupby(
        ["anon_polygon_id", "_year"], sort=False
    ):
        ids = group.index
        # Явная единица исключает расхождение масштаба datetime64[us]/[ns] в pandas.
        x = (
            pd.to_datetime(group.date) - pd.Timestamp("2000-01-01")
        ).dt.total_seconds().to_numpy() / 86400
        local = {}
        for sensor in SENSORS:
            values = pd.to_numeric(
                group.get(sensor, pd.Series(np.nan, index=ids)), errors="coerce"
            ).to_numpy(float)
            ...
            for side, offset in [
                ...
                if len(xx):
                    ...
                    local[f"{sensor}_{side}_days"] = np.where(
                        valid, np.abs(x - xx[pos]), np.nan
                    )
            ...
            for window in [8, 24, 48]:
                ...
                variance = np.maximum(
                    (squares[hi] - squares[lo]) / np.maximum(counts, 1) - avg * avg, 0
                )
                local[f"{sensor}_window_{window}_mean"] = np.where(counts, avg, np.nan)
                local[f"{sensor}_window_{window}_std"] = np.where(
                    counts, np.sqrt(variance), np.nan
                )
                local[f"{sensor}_window_{window}_count"] = counts
            # Межсенсорное смещение оценивается по одновременным видимым строкам.
            for other in SENSORS:
                if other == sensor:
                    continue
                ov = pd.to_numeric(
                    group.get(other, pd.Series(np.nan, index=ids)), errors="coerce"
                ).to_numpy(float)
                overlap = good & np.isfinite(ov) & (np.abs(ov) <= 1)
                diff = values[overlap] - ov[overlap]
                local[f"{sensor}_minus_{other}"] = np.full(
                    len(x), np.median(diff) if len(diff) >= 3 else np.nan
                )
...
def baselines(features):
    linear = features.primary_ndvi_linear.fillna(0.35).to_numpy(float)
    sensors = np.stack(
        [
            features[f"{s}_linear"]
            .fillna(pd.Series(linear, index=features.index))
            .to_numpy(float)
            for s in SENSORS
        ],
        axis=1,
...
def fit_bundle(
    ...
    general = HistGradientBoostingRegressor(
        max_iter=600,
        max_leaf_nodes=31,
    ...
    if experts:
        ...
        for source in range(3):
            ...
            model = HistGradientBoostingRegressor(
                ...
                learning_rate=0.035,
                early_stopping=False,
                random_state=seed + source,
                categorical_features=categorical_features,
            )
            source_x = features.loc[keep]
            # В подвыборке источника часть каналов может отсутствовать. Пустые и
            # константные признаки определяются только по обучающим строкам.
            useful = source_x.nunique(dropna=True) > 1
            weights = None if sample_weight is None else sample_weight[keep]
            model.fit(
                source_x.loc[:, useful],
                (target - candidates[:, source])[keep],
...
def predict_bundle(bundle, features):
    ...
    predictions = np.stack(
        [
            np.clip(
                candidates[:, i] + model.predict(x[list(model.feature_names_in_)]),
                -1,
                1,
            )
            for i, model in enumerate(bundle["experts"])
        ],
        axis=1,
    )
