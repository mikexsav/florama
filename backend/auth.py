...
def require_user(fn):
    ...
    def wrapped(*args, **kwargs):
        ...
        with transaction(current_app.config["DB_PATH"]) as c:
            row = c.execute(
                """SELECT u.*, s.csrf, s.token_hash FROM sessions s
...
def credentials(data):
    ...
    if not re.fullmatch(r"[^\s@]+@[^\s@]+\.[^\s@]+", email) or mode not in (
        "login",
        "register",
    ):
...
def send_code():
    ...
    # Фиксированный код упрощает проверку локального стенда без SMTP. create_app
    # запрещает эту настройку во всех окружениях, кроме development.
    code = current_app.config.get("DEV_OTP_CODE") or f"{secrets.randbelow(1000000):06d}"
    ...
    with transaction(current_app.config["DB_PATH"], immediate=True) as c:
        ...
        if eligible:
            c.execute(
                """INSERT INTO codes(email,code_hash,expires_at,attempts,last_sent_at,mode)
    ...
    if eligible and not current_app.config.get("DEV_OTP_CODE"):
        try:
            send_email(email, code)
        except Exception as exc:
            with transaction(current_app.config["DB_PATH"]) as c:
                c.execute(
                    "DELETE FROM codes WHERE email=? AND code_hash=?", (email, hashed)
                )
            current_app.logger.error(
                "smtp_failure type=%s request_id=%s", type(exc).__name__, g.request_id
            )
            return jsonify(
                error="Почтовый сервис недоступен. Повторите позже.",
                requestId=g.request_id,
            ), 502
    return jsonify(
        message="Если почта подходит для выбранного действия, код отправлен. Для существующего аккаунта выберите «Войти».",
        retryAfter=60,
    )
...
def verify_code():
    ...
    with transaction(current_app.config["DB_PATH"], immediate=True) as c:
        ...
        if (
            ...
            row["code_hash"], keyed(email + "|" + mode + "|" + code)
        ):
            ...
            if not user:
                ...
                c.execute(
                    ...
                    (digest(token), user["id"], csrf, now + 604800, now),
                )
    if error:
        raise BadRequest(
            "Неверный или просроченный код. Запросите новый код для выбранного действия."
        )
