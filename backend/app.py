        ...
        for row in csv.DictReader(stream):
            ...
            if any(value is not None for value in values.values()):
                by_date.setdefault(day, []).append(
                    {"id": aoi, "values": values, "crop": crops[aoi]}
                )
...
def csv_rows(upload):
    ...
    for index, row in enumerate(rows, start=2):
        ...
        try:
            if geometry_raw:
                ...
                west, south, east, north = (
                    float(values[key]) for key in ("west", "south", "east", "north")
                )
                geometry = {
                    "type": "Polygon",
                    "coordinates": [
                ...
                raise ValueError(
                    "нужна колонка geometry/geojson, wkt или west,south,east,north"
                )
        ...
        result.append(
            {
                "geo": geo,
                "name": values.get("name") or f"Участок {index - 1}",
                "region": values.get("region", ""),
                "crop": values.get("crop", ""),
                "cadastral": values.get("cadastralnumber")
                or values.get("cadastral_number", ""),
            }
        )
    return result
...
def create_app(config=None):
    ...
    app.config.update(
        ...
        DATA_DIR=os.getenv("DATA_DIR", "/opt/florama/data"),
        FLORAMA_ENV=os.getenv("FLORAMA_ENV", "production"),
        DEV_OTP_CODE=os.getenv("DEV_OTP_CODE"),
    )
    if config:
        app.config.update(config)
    if len(app.config["SECRET_KEY"]) < 32:
        raise RuntimeError(
            "Задайте APP_SECRET длиной не менее 32 символов в окружении сервера."
        )
    dev_code = app.config.get("DEV_OTP_CODE")
    if dev_code and (
        app.config["FLORAMA_ENV"] != "development"
        or not re.fullmatch(r"\d{6}", dev_code)
    ):
        raise RuntimeError(
            "DEV_OTP_CODE разрешён только в development и должен содержать 6 цифр."
        )
    migrate(app.config["DB_PATH"])
    # API слушает только loopback; ровно один доверенный nginx перед приложением.
    app.wsgi_app = ProxyFix(app.wsgi_app, x_for=1, x_proto=1)
    origins = os.getenv(
        "APP_ORIGINS", "https://crewloom.ru,https://www.crewloom.ru"
    ).split(",")
    CORS(
        app,
    ...
    def after(response):
        ...
        app.logger.info(
            ...
            round((time.monotonic() - g.get("started", time.monotonic())) * 1000),
        )
        return response
    ...
    def error(exc):
        if isinstance(exc, HTTPException):
            return jsonify(
                error=exc.description, requestId=g.get("request_id", "-")
            ), exc.code
        app.logger.exception(
            "request_id=%s unhandled type=%s",
            g.get("request_id", "-"),
            type(exc).__name__,
        )
        return jsonify(
            error="Ошибка сервера. Сохраните ID ошибки для диагностики.",
    ...
    def owned(c, table, object_id):
        row = c.execute(
            f"SELECT * FROM {table} WHERE id=? AND user_id=?", (object_id, g.user["id"])
        ).fetchone()
        if not row:
    ...
    def settings():
        with db() as c:
            row = c.execute(
                "SELECT settings FROM preferences WHERE user_id=?", (g.user["id"],)
            ).fetchone()
            values = {
                "restore": True,
                "anomaly": True,
                **(json.loads(row["settings"]) if row else {}),
            }
            if request.method == "PATCH":
                data = payload()
                if any(
                    k not in ("restore", "anomaly") or not isinstance(v, bool)
                    for k, v in data.items()
                ):
                    raise BadRequest(
                        "Параметры должны быть логическими restore/anomaly."
                    )
                values.update(data)
                c.execute(
                    "INSERT OR REPLACE INTO preferences VALUES (?,?)",
                    (g.user["id"], dumps(values)),
                )
            ...
            if request.method == "POST":
                ...
                c.execute(
                    ...
                    (
                        project_id,
                        g.user["id"],
                        text_field(data.get("name", ""), "Название", 120, True),
                        text_field(data.get("region", ""), "Регион", 200),
                        int(time.time()),
                    ),
                )
            values = [
                dict(v)
                for v in c.execute(
                    "SELECT id,name,region,created_at FROM projects WHERE user_id=? ORDER BY created_at DESC",
                    (g.user["id"],),
                )
            ]
    ...
    def polygons():
        if request.method == "POST":
            ...
            with db() as c:
                if (
                    c.execute(
                        "SELECT COUNT(*) FROM polygons WHERE user_id=?", (g.user["id"],)
                    ).fetchone()[0]
                    >= 100
                ):
                ...
                c.execute(
                    ...
                    (
                        ...
                        text_field(data.get("name", ""), "Название", 120, True),
                        text_field(data.get("region", ""), "Регион", 200),
                        text_field(data.get("crop", ""), "Культура", 80),
                        dumps(geo["geometry"]),
                        geo["area_ha"],
                        geo["latitude"],
                        geo["longitude"],
        ...
        with db() as c:
            values = [
                polygon_json(v)
                for v in c.execute(
                    "SELECT * FROM polygons WHERE user_id=? ORDER BY created_at DESC",
                    (g.user["id"],),
                )
            ]
            for item in values:
                last = c.execute(
                    "SELECT id,result FROM analyses WHERE polygon_id=? ORDER BY created_at DESC LIMIT 1",
                    (item["id"],),
                ).fetchone()
                if last:
                    item["analysisId"] = last["id"]
                    item["summary"] = json.loads(last["result"])["summary"]
    ...
    def delete_polygon(object_id):
        with db(immediate=True) as c:
            ...
            if c.execute(
                "SELECT 1 FROM jobs WHERE polygon_id=? AND status IN ('queued','running')",
                (object_id,),
            ).fetchone():
                raise BadRequest(
                    "Дождитесь завершения обработки перед удалением участка."
                )
    ...
    def import_polygons_csv():
        ...
        with db(immediate=True) as c:
            ...
            if existing + len(rows) > 100:
                raise BadRequest(
                    f"Лимит аккаунта: 100 участков. Можно добавить ещё {100 - existing}."
                )
            now = int(time.time())
            created = []
            for row in rows:
                ...
                c.execute(
                    ...
                    (
                        ...
                        g.user["id"],
                        project_id,
                        text_field(row["name"], "Название", 120, True),
                        text_field(row["region"], "Регион", 200),
                        text_field(row["crop"], "Культура", 80),
                        dumps(geo["geometry"]),
                        geo["area_ha"],
                        geo["latitude"],
                        geo["longitude"],
    ...
    def analyze(object_id):
        ...
        try:
                ...
                data.get("end", str(date.today() - timedelta(days=1)))
            )
            start = date.fromisoformat(data.get("start", str(end - timedelta(days=89))))
            if (
                not date(2019, 1, 1) <= start <= end < date.today()
        ...
        with db(immediate=True) as c:
            ...
            if previous:
                return jsonify(
                    jobId=previous["id"],
                    message="Уже выполняется задача. Дождитесь завершения.",
                ), 202
            job_id = uuid.uuid4().hex
            now = int(time.time())
            c.execute(
                "INSERT INTO jobs(id,user_id,polygon_id,kind,payload,created_at,updated_at) VALUES (?,?,?,?,?,?,?)",
                (
                    job_id,
                    g.user["id"],
                    object_id,
                    "monitor",
                    dumps({"start": str(start), "end": str(end)}),
                    now,
                    now,
                ),
            )
    ...
    def jobs():
        with db() as c:
            values = [
                dict(v)
                for v in c.execute(
                    "SELECT id,polygon_id,kind,status,progress,message,result,created_at,updated_at FROM jobs WHERE user_id=? ORDER BY created_at DESC LIMIT 30",
                    (g.user["id"],),
                )
            ]
        for item in values:
    ...
    def analysis(object_id):
        with db() as c:
            row = c.execute(
                "SELECT a.result FROM analyses a JOIN polygons p ON p.id=a.polygon_id WHERE a.id=? AND p.user_id=?",
                (object_id, g.user["id"]),
            ).fetchone()
        if not row:
    ...
    def regions():
        ...
        try:
            ...
            return jsonify(
                error="Поиск региона недоступен. Переместите карту вручную.",
    ...
    def fields():
        ...
        try:
            ...
            return jsonify(
                error="Поиск контуров недоступен. Можно нарисовать участок вручную.",
                requestId=g.request_id,
            ), 502
    ...
    @require_user
    def batch():
        limit("batch:" + str(g.user["id"]), 200, 3600)
        upload = request.files.get("file")
        if not upload or not upload.filename.lower().endswith(".csv"):
        ...
        with db(immediate=True) as c:
            if c.execute(
                "SELECT 1 FROM jobs WHERE user_id=? AND status IN ('queued','running')",
                (g.user["id"],),
            ).fetchone():
            ...
            now = int(time.time())
            c.execute(
                "INSERT INTO jobs(id,user_id,kind,payload,created_at,updated_at) VALUES (?,?,?,?,?,?)",
                (
                    job_id,
                    g.user["id"],
                    "batch",
    ...
    def research_map():
        ...
        if selected not in by_date:
            try:
                ...
                raise BadRequest(
                    "Дата карты должна быть в формате YYYY-MM-DD."
                ) from exc
            selected = min(
                dates, key=lambda item: abs((date.fromisoformat(item) - requested).days)
            )
        cells = []
        for row in sorted(by_date[selected], key=lambda item: item["id"]):
            cells.append(
                {"id": row["id"], "value": row["values"][layer], "crop": row["crop"]}
            )
        return jsonify(
            ...
            layers=[
                {"id": key, "label": label} for key, label in RESEARCH_LAYERS.items()
            ],
            cells=cells,
            aoiIds=sorted(crops),
            totalAoi=len(crops),
    ...
    def client_errors():
        ...
        app.logger.warning(
            "client_error user_id=%s kind=%s request_id=%s",
            g.user["id"],
            kind,
            g.request_id,
        )
