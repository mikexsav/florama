...
def ratio(a, b):
    return np.divide(
        a - b, a + b, out=np.full_like(a, np.nan), where=np.abs(a + b) > 1e-6
    )


def indices(bands):
    blue, red, nir, rededge, swir = [
        bands[k] for k in ["blue", "red", "nir", "rededge1", "swir16"]
    ]
    ...
    return {
        "ndvi": ratio(nir, red),
        "ndmi": ratio(nir, swir),
        "ndre": ratio(bands["nir08"], rededge),
        "bsi": ratio(swir + red, nir + blue),
        "evi": evi,
        "swir_ratio": np.divide(
            swir,
            bands["swir22"],
            out=np.full_like(nir, np.nan),
            where=bands["swir22"] > 1e-6,
        ),
    }
...
def reflectance(raw, asset, properties):
    ...
    if properties.get("earthsearch:boa_offset_applied") is True:
        offset = 0.0
    return raw * meta.get("scale", 0.0001) + offset
...
def analyze_scene(item, geometry, include_grid=False):
    ...
    with rasterio.Env(
        ...
        resolution = max(
            20, math.ceil(math.sqrt((east - west) * (north - south) / 65536) / 20) * 20
        )
        ...
        inside = geometry_mask(
            [mapping(projected)], out_shape=(height, width), transform=grid, invert=True
        )
        valid = inside.copy()
        for name in BANDS:
            asset = assets[name]
        ...
        result = {
            ...
            "sceneId": item["id"],
            "source": "ESA Sentinel-2 L2A / Earth Search",
            "tileCloudPercent": item["properties"].get("eo:cloud_cover"),
            "validFraction": round(fraction, 4),
            "resolutionM": resolution,
            "validPixels": int(valid.sum()),
            "usable": fraction >= 0.25 and int(valid.sum()) >= 3,
        }
        for key, value in values.items():
            eligible = valid & np.isfinite(value)
            result[key] = (
                round(float(np.median(value[eligible])), 5)
                if result["usable"] and eligible.any()
                else None
            )
        if not result["usable"] or not include_grid:
            return result
        vegetation = valid & (values["ndvi"] > 0.2)
        weak = (
            vegetation
            & (
                values["ndvi"]
                < max(0.3, float(np.median(values["ndvi"][valid])) - 0.15)
            )
            & (values["ndmi"] < 0.1)
        )
        ...
        for n in range(1, count + 1):
            ...
            lon, lat = unproject(
                west + (col + 0.5) * resolution, north - (row + 0.5) * resolution
            )
            zones.append(
                {
                    "lat": lat,
                    "lon": lon,
                    "areaHa": round(count_pixels * resolution**2 / 10000, 3),
                    "reason": "Совместное локальное снижение NDVI и NDMI; нужен осмотр, болезнь не подтверждена.",
                }
            )
        result["inspectionPoints"] = sorted(zones, key=lambda x: -x["areaHa"])[:5]
        result["stressAreaHa"] = round(int(weak.sum()) * resolution**2 / 10000, 3)
        result["vegetationFraction"] = round(
            float(vegetation.sum() / max(1, valid.sum())), 3
        )
        result["bareSoilFraction"] = round(
            float((valid & (bands["scl"] == 5)).sum() / max(1, valid.sum())), 3
        )
        ...
        for row in range(0, height, stride):
            for col in range(0, width, stride):
                block = np.s_[
                    row : min(row + stride, height), col : min(col + stride, width)
                ]
                good = valid[block]
                if not good.any():
                    continue
                cell = box(
                    west + col * resolution,
                    north - min(row + stride, height) * resolution,
                    west + min(col + stride, width) * resolution,
                    north - row * resolution,
                ).intersection(projected)
                if cell.is_empty:
                    continue
                props = {
                    k: round(
                        float(np.median(v[block][good & np.isfinite(v[block])])), 4
                    )
                    if (good & np.isfinite(v[block])).any()
                    else None
                    for k, v in values.items()
                }
                rgb = [
                    int(np.clip(np.median(bands[k][block][good]) * 2.5, 0, 1) * 255)
                    for k in ["nir", "red", "blue"]
                ]
                props["falseColor"] = "#" + "".join(f"{v:02x}" for v in rgb)
                cells.append(
                    {
                        "type": "Feature",
                        "geometry": mapping(transform_shape(unproject, cell)),
                        "properties": props,
                    }
                )
