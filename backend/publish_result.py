"""Recompute a user's existing CSV as a new, separately versioned job."""
import argparse
import hashlib
import json
import shutil
...
def main():
    ...
    if source is None:raise ValueError('Source job not found')
    source_path=Path(json.loads(source['payload'])['input'])
    frame=pd.read_csv(source_path)
    expected=frame.loc[gap_mask(frame),['anon_polygon_id','date']].reset_index(drop=True)
