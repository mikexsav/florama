...
def geocode(query):
    value = fetch_json('https://nominatim.openstreetmap.org/search',params={'q':query,'format':'jsonv2','limit':5,'accept-language':'ru'},ttl=86400)
    return [{'name':v['display_name'],'lat':float(v['lat']),'lon':float(v['lon']),'bbox':[float(x) for x in v['boundingbox']]} for v in value]


def farmland(bounds):
    west,south,east,north = bounds
    if not (-180<=west<east<=180 and -85<=south<north<=85) or east-west>.3 or north-south>.3:
        raise ValueError('Приблизьте карту: область поиска не более 0,3° по каждой стороне.')
        ...
        try:
            ...
            for item in data.get('elements',[]):
                ...
                if len(coords)<4 or coords[0]!=coords[-1]:
                    continue
                features.append({'type':'Feature','properties':{'osmId':item['id'],'name':item.get('tags',{}).get('name',f"Участок OSM {item['id']}"),'source':'OpenStreetMap','landuse':item.get('tags',{}).get('landuse')},'geometry':{'type':'Polygon','coordinates':[coords]}})
            return {'type':'FeatureCollection','features':features,'attribution':'© OpenStreetMap contributors, ODbL','limit':100}
        except requests.RequestException as exc:
            error = exc
    raise RuntimeError('Поиск OSM временно недоступен. Можно нарисовать контур самостоятельно.') from error
...
def weather(lat, lon, start, end):
    ...
    if date.fromisoformat(start)<=archive_end:
        data = fetch_json('https://archive-api.open-meteo.com/v1/archive',params={'latitude':lat,'longitude':lon,'start_date':start,'end_date':str(archive_end),'daily':fields,'models':'era5','timezone':'UTC'},ttl=86400)
        daily = data.get('daily',{})
        for i,day in enumerate(daily.get('time',[])):
            rows[day] = {'date':day,**{k:daily[k][i] for k in fields.split(',') if k in daily},'source':'ERA5 / Open-Meteo','timezone':'UTC'}
    if date.fromisoformat(end)>archive_end:
        data = fetch_json('https://api.open-meteo.com/v1/forecast',params={'latitude':lat,'longitude':lon,'daily':fields,'past_days':7,'forecast_days':1,'timezone':'UTC'},ttl=3600)
        daily=data.get('daily',{})
        for i,day in enumerate(daily.get('time',[])):
            if start<=day<=end and day not in rows:
                rows[day]={'date':day,**{k:daily[k][i] for k in fields.split(',') if k in daily},'source':'Open-Meteo operational model','timezone':'UTC'}
...
def scenes(geometry,start,end,limit=24):
    ...
    for f in features:
        ...
        if day not in by_day or f['properties'].get('eo:cloud_cover',100)<by_day[day]['properties'].get('eo:cloud_cover',100):
            by_day[day]=f
