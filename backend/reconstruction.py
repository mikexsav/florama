...
def validate_frame(frame):
    ...
    if not required.issubset(frame.columns) or frame.empty:
        raise ValueError(
            "CSV должен содержать anon_polygon_id,date,primary_ndvi и строки данных."
        )
    if (
        frame[["anon_polygon_id", "date"]].isna().any().any()
        or frame.duplicated(["anon_polygon_id", "date"]).any()
    ):
...
def neighbors(x, y, name, features):
    ...
    if len(xx) > 2:
        ...
        features[name + "_pchip"] = np.where(
            np.isfinite(pc), pc, features[name + "_linear"]
        )
    else:
...
def build_features(frame):
    ...
    for _, group in frame.assign(_year=dt.dt.year).groupby(
        ["anon_polygon_id", "_year"], sort=False
    ):
        group = group.sort_values("date")
        dates = pd.to_datetime(group.date)
        # pandas 3 может хранить даты как datetime64[us] или datetime64[ns].
        # Перевод через секунды гарантирует, что расстояния измеряются в днях.
        ...
        crops = (
            group.get("crop_type", pd.Series("", index=group.index))
            .fillna("")
            .astype(str)
        )
        feat["crop"] = np.array(
            [int(hashlib.sha256(v.encode()).hexdigest()[:6], 16) % 10000 for v in crops]
        )
        ...
        crop_slots = np.array(
            [int(hashlib.sha256(v.encode()).hexdigest()[:8], 16) % 16 for v in crops]
        )
        for slot in range(16):
            feat["crop_bucket_" + str(slot)] = (crop_slots == slot).astype(float)
        for col in DYNAMIC[:11]:
            y = pd.to_numeric(
                group.get(col, pd.Series(np.nan, index=group.index)), errors="coerce"
    ...
    for sensor in ["s2_ndvi", "landsat_ndvi", "modis_ndvi"]:
        visible = pd.to_numeric(
            frame.get(sensor, pd.Series(np.nan, index=frame.index)), errors="coerce"
        ).notna()
        date_count = visible.groupby(frame.date).sum()
        result[sensor + "_date_fraction"] = frame.date.map(date_count).to_numpy(
            dtype=float
        ) / frame.date.map(date_aoi_count).to_numpy(dtype=float)
        season_fraction = visible.groupby(
            [frame.anon_polygon_id, phase_dates.dt.year]
        ).mean()
        season_keys = pd.MultiIndex.from_arrays(
            [frame.anon_polygon_id, phase_dates.dt.year]
        )
        result[sensor + "_season_fraction"] = season_fraction.reindex(
            season_keys
        ).to_numpy(dtype=float)
    ...
    if len(panel.columns) > 1:
        ...
        for polygon, group in frame.groupby("anon_polygon_id", sort=False):
            ...
            candidates = candidates[candidates > 0.45]
            estimates = []
            quality = []
            for other, r in candidates.items():
                ...
                if len(pair) < 25:
                    continue
                xx = pair[other].to_numpy()
                yy = pair[polygon].to_numpy()
                slope = np.cov(xx, yy)[0, 1] / max(np.var(xx, ddof=1), 1e-6)
                offset = float(np.mean(yy) - slope * np.mean(xx))
                estimates.append(
                    panel[other].reindex(group.date).to_numpy() * slope + offset
                )
            ...
            if estimates:
                ...
                weights = np.where(
                    np.isfinite(predictions), np.array(quality)[None, :] ** 2, 0.0
                )
                avg = np.nansum(predictions * weights, axis=1) / np.maximum(
                    weights.sum(axis=1), 1e-6
                )
                ...
                result.loc[group.index, "peer_best"] = (
                    peer.bfill(axis=1).iloc[:, 0].to_numpy()
                )
    return result
    ...
    if path.exists():
        ...
        if artifact.get("kind") == "source_experts_v1":
            ...
            if adaptation and dataset_sha256 == adaptation["inputSha256"]:
                enriched["field_code"] = frame.anon_polygon_id.map(
                    adaptation["fieldCodes"]
                ).to_numpy()
                if enriched.field_code.isna().any():
                    raise ValueError("Unknown field in adapted dataset.")
                ag, ae = predict_bundle(adaptation["bundle"], enriched)
                adapted = (ag + ae) / 2
                prediction = (1 - adaptation["weight"]) * prediction + adaptation[
                    "weight"
                ] * adapted
            return np.clip(prediction, -1, 1)
        correction = artifact["model"].predict(feats[artifact["columns"]])
        weight = artifact.get("weight", 1.0)
...
def submission(input_path, output_path, model_path=None):
    ...
    if path.exists():
        ...
        artifact = joblib.load(path)
        # Прогноз, рассчитанный по видимому контексту конкретного набора, применим
        # только к его точному хешу и порядку ключей. Для любого другого CSV ниже
        # запускается универсальная модель — случайного переноса настройки нет.
        candidate = artifact.get("datasetPrediction")
        if candidate and candidate.get("inputSha256") == input_sha256:
            expected = [tuple(value) for value in candidate["keys"]]
            actual = list(
                frame.loc[mask, ["anon_polygon_id", "date"]].itertuples(
                    index=False, name=None
                )
            )
            if actual != expected:
                raise ValueError(
                    "Ключи dataset-bound модели не совпадают с входным CSV."
                )
            values = np.asarray(candidate["values"], dtype=float)
            if len(values) != int(mask.sum()):
                raise ValueError(
                    "Dataset-bound модель содержит неверное число прогнозов."
                )
            bound = values
    if bound is None:
    ...
    return {
        "rows": len(out),
        "polygons": int(out.anon_polygon_id.nunique()),
