"""Трансдуктивный LightGBM с видимой историей целевых полей как контекстом."""
...
def main():
    ...
    for fold in range(4):
        ...
        visible = test.copy()
        visible.loc[eval_mask, "primary_ndvi"] = np.nan
        ...
        tx = tx.copy()
        tx["field_code"] = selected_ids(test, 9001 + fold).astype("int32")
        weights = np.r_[
            np.ones(sum(len(p[1]) for p in pieces)),
            np.full(sum(len(p[1]) for p in adaptive), 4.0),
        ]
        ...
        model.fit(
            x, y - linear, sample_weight=weights, categorical_feature=["field_code"]
        )
        direct = np.clip(tl + model.predict(tx), -1, 1)
        predictions = {"v15": reference, "adaptive_lgbm": direct}
        for weight in [0.2, 0.35, 0.5, 0.65, 0.8]:
            predictions[f"blend_{weight}"] = (1 - weight) * reference + weight * direct
        folds[str(fold)] = {k: score(ty, v) for k, v in predictions.items()}
        print(fold, json.dumps(folds[str(fold)]), flush=True)
        truths.append(ty)
        for name, pred in predictions.items():
            pooled.setdefault(name, []).append(pred)
        joblib.dump(
            {"truth": ty, "predictions": predictions},
            out / f"adaptive_lgbm_{fold}.joblib",
        )
    aggregate = {
        k: score(np.concatenate(truths), np.concatenate(v)) for k, v in pooled.items()
    }
    report = {"folds": folds, "aggregate": aggregate}
    (out / "adaptive_lgbm_report.json").write_text(json.dumps(report, indent=2))
