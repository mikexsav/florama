"""Test adaptation exclusively on visible target-field labels."""
import argparse
import json
...
def main():
    ...
    for fold,holdout in enumerate(np.array_split(ids,4)):
        ...
        # Identical evaluation mask to benchmark_sources, removed BEFORE making
        # any adaptation examples, including all satellite and weather values.
        ...
        base=HistGradientBoostingRegressor(max_iter=600,max_leaf_nodes=31,l2_regularization=20,learning_rate=.035,early_stopping=False,random_state=42)
        ...
        for weight in [1,3]:
            ...
            yy=np.r_[y,ay];ll,_=baselines(xx)
            model=HistGradientBoostingRegressor(max_iter=600,max_leaf_nodes=31,l2_regularization=20,learning_rate=.035,early_stopping=False,random_state=42)
            model.fit(xx,yy-ll,sample_weight=np.r_[np.ones(len(x)),np.full(len(ax),weight)])
        ...
        all_y.append(ty)
        for name,pred in predictions.items():all_predictions.setdefault(name,[]).append(pred)
        (out/'adaptation_report.json').write_text(json.dumps({'folds':results},indent=2))
    aggregate={k:score(np.concatenate(all_y),np.concatenate(v)) for k,v in all_predictions.items()}
    report={'folds':results,'aggregate':aggregate,'validation':'Evaluation rows permanently masked before adaptation; only visible test-field labels used.'}
