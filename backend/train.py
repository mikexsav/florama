...
def train(path, output):
    ...
    known = (
        frame.primary_ndvi.notna().to_numpy()
        & frame.primary_ndvi.between(-1, 1).to_numpy()
    )
    ...
    for seed in seeds:
        ...
        linear = np.nan_to_num(feats.primary_ndvi_linear.to_numpy(), nan=0.35)
        datasets.append((feats, target, keys, linear))
        print("features", seed, len(target), flush=True)
    feats, target, keys, linear = datasets[0]
    mean = np.nan_to_num(feats.primary_ndvi_mean.to_numpy(), nan=0.35)
    pchip = np.nan_to_num(feats.primary_ndvi_pchip.to_numpy(), nan=0.35)
    unseen = sorted(frame.anon_polygon_id.unique())[-8:]
    splits = {
        "new_polygons": keys.anon_polygon_id.isin(unseen).to_numpy(),
        "new_season": keys.date.str.startswith("2024").to_numpy(),
    }
    report = {
        "seeds": seeds,
        "maskedFraction": 0.15,
    ...
    for name, test in splits.items():
        ...
        variants = {
            "nearest_mean": scores(target[test], mean[test]),
            "linear": scores(target[test], linear[test]),
            "pchip": scores(target[test], pchip[test]),
        }
    ...
    joblib.dump(
        ...
        output / "gap_model.joblib",
    )
    report["selectedWeight"] = weight
    report["evaluationNote"] = (
        "Метрики на искусственных пропусках reference train, не скрытая метрика организаторов. Вес выбран по этим validation splits."
    )
    report["model"] = {
        "algorithm": "Multi-mask HistGradientBoosting residual + linear + crop and orbit features",
        "max_iter": 500,
        "max_leaf_nodes": 31,
        "l2_regularization": 20,
        "learning_rate": 0.035,
        "trainingExamples": len(all_y),
    }
