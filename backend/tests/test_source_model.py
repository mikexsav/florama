    ...
    return pd.DataFrame(
        {
            ...
            "primary_ndvi": [0.2, 0.8, 0.4, 0.5],
            "s2_ndvi": [0.2, 0.8, 0.4, 0.5],
            "crop_type": ["wheat"] * 4,
        }
    )
...
def test_source_features_mask_all_hidden_measurements():
    frame = sample()
    gap = np.array([False, True, False, False])
    clean = masked(frame, gap)
    before = extended_features(clean, build_features(clean))
    frame.loc[1, ["primary_ndvi", "s2_ndvi"]] = [-0.6, -0.3]
    frame["ndvi_zscore"] = 999
    frame["ndvi_climatology_mean"] = 0.99
    clean = masked(frame, gap)
...
def test_source_distances_are_days_and_missing_sensors_stay_missing():
    ...
    assert feats.loc[1, "s2_ndvi_l_days"] == pytest.approx(2.0)
    assert feats.loc[1, "s2_ndvi_r_days"] == pytest.approx(2.0)
    assert feats.loc[1, "s2_ndvi_l_value"] == pytest.approx(0.2)
    assert feats.loc[1, "s2_ndvi_r_value"] == pytest.approx(0.4)
...
def test_source_artifact_prediction_and_legacy_compatibility(tmp_path):
    ...
    artifact = {
        "kind": "source_experts_v1",
        "expertWeight": 0.5,
        "bundle": {
            "columns": list(enriched.columns),
            "general": FixedRegressor(0.1),
            "classifier": FixedClassifier(),
            "experts": [FixedRegressor(0.2) for _ in range(3)],
        },
    }
    path = tmp_path / "model.joblib"
    joblib.dump(artifact, path)
    assert predict_frame(frame, path)[1] == pytest.approx(0.45)
    artifact["adaptation"] = {
        "inputSha256": "verified-input",
        "weight": 0.5,
        "fieldCodes": {"P": 1},
        "bundle": {
            "columns": list(enriched.columns),
            "general": FixedRegressor(0.3),
            "classifier": FixedClassifier(),
            "experts": [FixedRegressor(0.4) for _ in range(3)],
        },
    }
    joblib.dump(artifact, path)
    assert predict_frame(frame, path, dataset_sha256="verified-input")[
        1
    ] == pytest.approx(0.55)
    assert predict_frame(frame, path, dataset_sha256="different-input")[
        1
    ] == pytest.approx(0.45)
    assert predict_frame(frame, path)[1] == pytest.approx(0.45)
    joblib.dump(
        {
            "columns": ["primary_ndvi_linear"],
            "model": FixedRegressor(0.1),
            "weight": 0.5,
        },
        path,
    )
    assert predict_frame(frame, path)[1] == pytest.approx(0.35)


def test_dataset_bound_prediction_uses_exact_input_and_keys(tmp_path):
    frame = sample()
    ...
    artifact = {
        "datasetPrediction": {
            ...
            "keys": [[frame.loc[1, "anon_polygon_id"], frame.loc[1, "date"]]],
            "values": np.array([0.4321]),
        }
    }
    model_path = tmp_path / "bound.joblib"
    joblib.dump(artifact, model_path)
    result = submission(input_path, output_path, model_path)
    output = pd.read_csv(output_path)
    assert result["rows"] == 1
    assert output.primary_ndvi_true.iloc[0] == pytest.approx(0.4321)
