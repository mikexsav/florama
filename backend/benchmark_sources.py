"""Disjoint-field benchmark. Private ground truth is never loaded."""
import argparse
import json
...
def examples(frame,seed):
    ...
    mask=valid & (np.random.default_rng(seed).random(len(frame))<.15)
    ...
    source=np.where(original.s2_ndvi.notna(),0,np.where(original.landsat_ndvi.notna(),1,2))
    return features,target,source


def score(y,p):
    return {'rmse':float(np.sqrt(np.mean((y-p)**2))),'mae':float(np.mean(np.abs(y-p))),'n':len(y)}
...
def main():
    ...
    for fold,holdout in enumerate(folds):
        ...
        print('fold',fold,'train_fields',train.anon_polygon_id.nunique(),'test_fields',len(holdout),flush=True)
        ...
        y=np.concatenate([p[1] for p in pieces]); labels=np.concatenate([p[2] for p in pieces])
        tx,ty,_=test_examples
        base_columns=[c for c in x if c not in set(x.columns)-set(build_features(train.head(1)).columns)]
        # Old and current training recipes see exactly the same validation rows.
        old_columns=[c for c in base_columns if not c.startswith('phase_') and not c.endswith(('_date_fraction','_season_fraction'))]
        old=HistGradientBoostingRegressor(max_iter=350,max_leaf_nodes=23,l2_regularization=10,learning_rate=.045,random_state=42)
        ox,oy,_=pieces[0];ol,_=baselines(ox)
        old.fit(ox[old_columns],oy-ol)
        tl,_=baselines(tx)
        old_prediction=np.clip(tl+old.predict(tx[old_columns]),-1,1)
        current=HistGradientBoostingRegressor(max_iter=500,max_leaf_nodes=31,l2_regularization=20,learning_rate=.035,random_state=42)
        ...
        predictions={'old_recipe':old_prediction,'v14':current_prediction,'extended':general,'experts':source}
        for w in [.25,.5,.75]:
            predictions[f'expert_blend_{w}']=(1-w)*general+w*source
        reports[str(fold)]={name:score(ty,pred) for name,pred in predictions.items()}
        print(json.dumps(reports[str(fold)]),flush=True)
        truths.append(ty)
        for name,pred in predictions.items():pooled.setdefault(name,[]).append(pred)
        joblib.dump({'truth':ty,'predictions':predictions,'holdout':holdout},out/f'fold_{fold}.joblib')
        (out/'report.json').write_text(json.dumps({'folds':reports},indent=2))
    aggregate={name:score(np.concatenate(truths),np.concatenate(pred)) for name,pred in pooled.items()}
    report={'folds':reports,'aggregate':aggregate,'validation':'Four disjoint AOI folds; held-out fields excluded from training features; fresh masks; no private ground truth.'}
