...
for name, path in [
    ("train", root / "train_dataset.csv"),
    ("private", Path("E:/doc-1788536731.csv")),
]:
    df = pd.read_csv(path)
    print(name, df.shape, list(df.columns))
    print(
        "polygons", df.anon_polygon_id.nunique(), "range", df.date.min(), df.date.max()
    )
    print("nonnull", df.count().to_dict())
    print(
        "gaps",
        df.is_synthetic_gap.value_counts().to_dict()
